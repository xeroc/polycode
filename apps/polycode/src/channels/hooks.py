...
class ChannelHooks:
    ...
    @hookimpl
    def on_flow_error(self, flow_id: str, state: Any, error: Exception) -> bool | None:
        """Send error notification on flow failure.
        ...
        Returns:
            None to let exception propagate
        """
        if (
            not hasattr(state, "project_config") or not state.project_config  # type: ignore[attr-defined]
        ):
        ...
        notification = Notification(
            content=f"Flow error: {error}",
            level=NotificationLevel.ERROR,
            context={"issue_id": getattr(state, "issue_id", None)},
            metadata={"flow_id": flow_id, "event": "error"},
        )
