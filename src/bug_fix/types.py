...
class BugFixState(BaseModel):
    ...
    affected_area: str = Field(
        default="", description="Affected files/modules"
    )
    reproduction: str = Field(default="", description="Reproduction steps")
    problem_statement: str = Field(
        default="", description="Clear problem description"
    )

    root_cause: Optional[str] = Field(
        default=None, description="Root cause analysis"
    )
    fix_approach: Optional[str] = Field(
        default=None, description="Proposed fix approach"
    )
    ...
    regression_test: Optional[str] = Field(
        default=None, description="Regression test added"
    )
